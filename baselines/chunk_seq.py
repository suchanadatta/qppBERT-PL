...
def main():
            ...
            for query in list(queries):
                ...
                for curr_window in res_window:
                    if win_count < args.chunk_per_query:
                        ...
                        win_count += 1
                        yield query.text, [texts[did] for did in list(curr_window)], len(numrel)
            ...
            for query in rng.sample(list(test_queries), k=len(list(test_queries))):
                ...
                for curr_window in res_window:
                    if win_count < len(res)/args.batch_size:
                        ...
                        texts = {d.doc_id: d.text for d in dataset.docs.lookup(list(curr_window)).values()}
                        win_count += 1
                        yield query, list(curr_window), [texts[did] for did in list(curr_window)]
    ...
    with logger.pbar_raw(total=args.train_its, ncols=200) as pbar:
        ...
        for train_i in range(len(train_queries) * args.chunk_per_query):
            query, docs, numrel = next(train_iter)
            count += 1
            print('**** : ', query)
            print('%%%% : ', numrel)
            inputs_train = tokeniser([query for _ in docs], [doc for doc in docs], padding=True,
                                     ...
                                     return_tensors='pt')
            utility = model(**{k: v for k, v in inputs_train.items()})
