...
def main():
    ...
    parser.add_argument('--chunk-per-query', default=1, type=int)
    ...
    model = BertQppModel()
    model_bi = SentenceTransformer('bert-base-uncased')
    ...
    with logger.pbar_raw(total=args.train_its, ncols=200) as pbar:
        ...
        for train_i in range(len(train_queries) * args.chunk_per_query):
            ...
            input_query = model_bi.encode(query, convert_to_tensor=True)
