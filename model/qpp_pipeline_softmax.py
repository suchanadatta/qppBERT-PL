...
def main():
    ...
    parser.add_argument('--index', default='path_of_the_pisa_index')
    parser.add_argument('--query', default='path_of_the_query_file')
    ...
    parser.add_argument('--outfile', default='path_of_the_output_file')
    ...
    print('Total %d train queries loaded : ' %len(list(train_queries)))
    print('Total %d dev queries loaded : ' %len(list(dev_queries)))
    eval_queries = list(dataset_eval.queries)
    print('Total %d test queries loaded : ' %len(list(eval_queries)))
    qrels = dataset_train.qrels.asdict()
    print('Qrels are loaded..')
    ...
    def _test_iter():
        while True:
            for query in rng.sample(list(eval_queries), k=len(list(eval_queries))):
                print('Current query : ', query.text)
        ...
        for train_i in range(len(train_queries) * args.chunk_per_query):
            ...
            if train_i % 20000 == 0:
                ...
                for valid_i in range(len(dev_queries) * args.chunk_per_query):
                    ...
                    inputs_dev = tokeniser([query for _ in docs], [doc for doc in docs], padding=True,
                                             truncation='only_second',
                                             return_tensors='pt')
                    utility = model(poshit, **{k: v for k, v in inputs_dev.items()})
    ...
    for curr_win in res_win:
        ...
        print(values)
        for i in range(1, args.chunk_per_query+1):
